...
export async function createUser(payload: { username: string; password: string; deptId?: number | string }) {
  const res = await client.post('/api/v1/users', payload); return res.data;
}
export async function assignRoles(userId: number | string, roleIds: (number | string)[]) {
...
export async function createDept(payload: { deptCode: string; deptName: string; parentId?: number | string }) {
...
export async function assignRolePermissions(roleId: number | string, permissionIds: (number | string)[]) {
