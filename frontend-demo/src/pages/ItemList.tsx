...
export default function ItemList() {
  ...
  const handleCreate = async (values: Record<string, unknown>) => {
    setSubmitting(true);
    try {
      const res = await createItem(values as { name: string; spec: string; unit: string; sku: string });
      const created = res.data;
      setItems((prev) => [...prev, { id: created?.id ?? Date.now(), ...values as unknown as Item }]);
